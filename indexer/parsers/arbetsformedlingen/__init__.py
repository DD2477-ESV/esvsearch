...
def parse(args):
    ...
    docs = crawl_site(cache_dir)
    console.log(f'found {len(docs)} docs')

    fetch_titles(docs, cache_dir)
    fetch_pdfs(docs, cache_dir)
    parse_text(docs, cache_dir)
    parse_dates(docs, cache_dir)
    index_documents(docs, cache_dir, ELASTICSEARCH_ENDPOINT, INDEX_NAME)


def crawl_site(cache_dir):
